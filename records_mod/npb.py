...
def create_team_list():
    ...
    for leag_initial in LEAG_INITIAL_LIST:
        ...
        header_tds = trs[1].find_all(
            'td')
        header = [
            header_td.text.replace('\u3000', '').replace('｜', 'ー')
            for header_td in header_tds[:-TEAM_NUM]
            if header_td.text
        ]
...
def update_hitter_y_records(hitter_list):
...
def update_pitcher_y_records(pitcher_list):
...
def update_records_by_official():
    ...
    pitcher_list = update_pitcher_y_records(pitcher_list)
    hitter_list = update_hitter_y_records(hitter_list)
